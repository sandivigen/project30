	...
	<div class="lang-sel sel-dropdown">
		...
		if(!function_exists('dfd_language_selector_flags')) {
			function dfd_language_selector_flags() {
				global $dfd_ronneby;
				$switcher_html = $flag_html = $active_switcher_html = '';
				if (function_exists('icl_get_languages')) {
					$languages = icl_get_languages('skip_missing=0&orderby=code');

					if (!empty($languages)) {
						foreach ($languages as $l) {
							$active_switcher_html = '<a href="#"><span>'.$l['translated_name'].'</span></a>';
							if(isset($dfd_ronneby['extra_header_options']) && $dfd_ronneby['extra_header_options'] == 'on') {
								...
								$active_switcher_html = '<a href="'.esc_url($l['url']).'"><span class="flag" style="background: transparent url('.$l['country_flag_url'].') center center no-repeat;"></span><span>'.$l['translated_name'].'</span></a>';
							}
							$switcher_html .= '<li>';

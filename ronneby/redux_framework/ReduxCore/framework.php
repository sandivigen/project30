...
// set()
             ...
             *
             * @since   3.5.4.3 and theme version 2.0.2
             * @return  converted option
             */
			private function convert($options) {
				if(!$options || empty($options)) return false;
				
				if(!isset($options['options_values'])) return $options;
				...
				if(isset($options['import_link']) && !empty($options['import_link']))
					$result['import_link'] = $options['import_link'];
				
				if(isset($options['import_code']) && !empty($options['import_code']))
					$result['import_code'] = $options['import_code'];
            ...
            public function set_options( $value = '' ) {
                ...
                if ( ! empty ( $value ) ) {
                    ...
                    if ( $this->args['database'] === 'transient' ) {
						...
						///disable save if we have errors. Added in 11.11.2015
				...
				// Store the changed values in the transient
                    ...
                    $this->options = $this->convert($value);
...
// set_options()
            ...
            public function get_options() {
                ...
                if ( empty ( $result ) && ! empty ( $defaults ) ) {
					...
					$this->options = $this->convert($result);
                }
...
//print_r($plugin_options);
				...
				if(isset($plugin_options['options_values'])) {
					$this->transients['options_values'] = $plugin_options['options_values'];

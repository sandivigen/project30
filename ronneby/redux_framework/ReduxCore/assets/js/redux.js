...
(function( $ ) {
	...
	/*
	$.redux.setTabValues = function () {
		var values = $('#option-values-collector').val(),
			optionsArray = [];
		
		if(values != '') {
			optionsArray = values.split(';');
			
			optionsArray.forEach(function(item, i, arr) {
				if(item != '') {
					var itemArr = item.split('~');
					

					if(itemArr[1] != '' && $('[name="'+itemArr[0]+'"]').length > 0) {
						//$('body').trigger('check_dependencies', $('[name="'+itemArr[0]+'"]')[0]);
						if($('[name="'+itemArr[0]+'"]').attr('type') == 'radio') {
							$('[name="'+itemArr[0]+'"]:not([value="'+itemArr[1]+'"])').prop('checked', false).removeAttr('checked');
							$('[name="'+itemArr[0]+'"][value="'+itemArr[1]+'"]').prop('checked', true).attr('checked', 'checked').trigger('change');
						} else {
							$('[name="'+itemArr[0]+'"]').val(itemArr[1]).trigger('change');
						}
					}
				}
			});
	...
	*/
	...
	$.redux.getValues = function () {
		...
		/*
		$('.redux-container-color input[type="text"].redux-color, .redux-container-select select, .redux-container-textarea textarea, .redux-container-text input[type="text"], .redux-container-slider input[type="text"], ').each(function() {
			var $self = $(this),
				name = $self.attr('name'),
				val = $self.val();
		...
		$('.redux-container-button_set input[type="radio"]:checked').each(function() {
			var $self = $(this),
				name = $self.attr('name'),
				val = $self.val();
		...
		$('.redux-container-color_gradient .redux-color').each(function() {
			var $self = $(this),
				name = $self.attr('name'),
				val = $self.val();
		...
		$('.redux-container-background .redux-color, .redux-container-background .redux-background-input:not(.color-transparency), .redux-container-background .redux-select-item, .redux-container-background .upload-id, .redux-container-background .upload-height, .redux-container-background .upload-width, .redux-container-background .upload-thumbnail').each(function() {
			var $self = $(this),
				name = $self.attr('name'),
				val = $self.val();
		...
		$('.redux-container-media .upload-id, .redux-container-media .upload, .redux-container-media .upload-height, .redux-container-media .upload-width, .redux-container-media .upload-thumbnail').each(function() {
			var $self = $(this),
				name = $self.attr('name'),
				val = $self.val();
		...
		$('.redux-container-typography .redux-typography-font-family, .redux-container-typography .typography-font-weight, .redux-container-typography .typography-font-style, .redux-container-typography .redux-typography-font-options, .redux-container-typography .redux-typography-google, .redux-container-typography .typography-subsets, .redux-container-typography .typography-font-size, .redux-container-typography .typography-line-height, .redux-container-typography .typography-letter-spacing, .redux-container-typography .redux-typography-color').each(function() {
			var $self = $(this),
				name = $self.attr('name'),
				val = $self.val();
		...
		*/
		$('.redux-field input[name]:not(#option-values-collector):not([type="radio"]), .redux-container-button_set input[name][type="radio"]:checked, .redux-field:not(.redux-container-import_export) textarea[name], .redux-field select[name]').each(function() {
			var $self = $(this),
				name = $self.attr('name'),
				val = $self.val();

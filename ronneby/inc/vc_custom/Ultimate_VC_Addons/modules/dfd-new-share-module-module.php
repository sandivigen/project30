	...
	class Dfd_New_Share_Module {
		...
		function dfd_new_share_module_init() {
			if (function_exists('vc_map')) {
				$module_images = get_template_directory_uri() . '/inc/vc_custom/Ultimate_VC_Addons/admin/img/share_module/';
				vc_map(
						array(
...
//								),
		...
		function dfd_new_share_module_shortcode($atts) {
			...
			foreach ($share_data as $key => $value) {
				...
				if ($$social_network) {
					echo '<li>';
					echo '<a class="module-entry-share-link-' . esc_attr($key) . ' feature-title" data-title="' . esc_attr($data_title) . '" data-url="' . esc_url($data_link) . '" data-media="" href="#"  ' . $single_style . '>' . $link_text . '</a>';
					echo '</li>';
				}
			}
			echo '</ul>';

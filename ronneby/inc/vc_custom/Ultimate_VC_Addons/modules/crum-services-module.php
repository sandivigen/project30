...
if ( ! class_exists( 'Dfd_Serveces_New' ) ) {
	class Dfd_Serveces_New {
		...
		function __construct() {
			...
			add_action( 'vc_before_init', array( &$this, 'dfd_new_service_init' ) );
		...
		function dfd_new_service_init() {
			if ( function_exists( 'vc_map' ) ) {
				$module_images = get_template_directory_uri() . '/inc/vc_custom/Ultimate_VC_Addons/admin/img/service_item/';
				//Dfd_service_new
				vc_map(
					array(

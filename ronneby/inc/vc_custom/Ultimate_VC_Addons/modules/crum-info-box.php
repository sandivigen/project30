...
if ( ! class_exists( 'Dfd_Info_Box' ) ) {
	...
	class Dfd_Info_Box {
		...
		function __construct() {
			add_action( 'vc_before_init', array( &$this, 'info_box_init' ) );
		...
		function info_box_init() {
			...
			if ( function_exists( 'vc_map' ) ) {
				$module_images = get_template_directory_uri() . '/inc/vc_custom/Ultimate_VC_Addons/admin/img/info_box/';
...
//			), $atts ) );
			...
			if ( isset( $readmore_show ) && 'yes' === $readmore_show ) {
				if ( $link && 'more' === $read_more ) {
					$link = vc_build_link( $link );
					$more_open = '<div><div class="dfd-module-readmore"><a href="' . $link['url'] . '" class="' . $readmore_style . '">';

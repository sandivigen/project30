	...
	class Dfd_Videoplayer {
		...
		function dfd_videoplayer_init() {
			if ( function_exists( 'vc_map' ) ) {
				$module_images = get_template_directory_uri() . '/inc/vc_custom/Ultimate_VC_Addons/admin/img/videoplayer/';
...
//									'heading'          => esc_html__( 'Round button?', 'dfd' ),
		...
		function dfd_videoplayer_shortcode( $atts ) {
			$main_style = $main_layout = $button_align = $title = $subtitle = $video_thumb = $video_animation =  $video_link = $title_font_options = $shadow =
			...
			if($lightbox_fill_color_start || $lightbox_fill_color_end){
				...
				$link_css .= '.dfd-fullscreen-video-container.video_module {'.$lightbox_style.'}';
			...
			if (('layout-1' === $main_layout) || ('layout-2' === $main_layout)) {
				...
				$button_html .= '<a href="#'.$unique_id.'" class="dfd-video-link '.$an_appear_class.'" data-animation="'.$video_animation.'"></a>';
				$button_html .= '</span>';
			} else {
...
//				$content_html .= '<div class="dfd-fullscreen-video-container" id="' . esc_html( $unique_id ) . '" data-animation="' . $video_animation . '">' . $embed . '<a href="#close-video" class="fullscreen-video-close"></a></div>';
				$content_html .= '<div  style="display:none;" class="dfd-vimeo-bg '.$an_appear_class.'" id="' . esc_html( $unique_id ) . '" data-animation="' . $video_animation . '">' . $embed . '</div>';
			}

...
if(!class_exists('Dfd_New_Gallery_Module')) {
	class Dfd_New_Gallery_Module {
		...
		var $admin_src = 'inc/vc_custom/Ultimate_VC_Addons/admin/img/gallery/';
		var $front_template = 'inc/vc_custom/Ultimate_VC_Addons/templates/gallery/';
...
//								),
		...
		function dfd_gallery_module_shortcode($atts) {
			$output = $title_html = $data_atts = $article_data_atts = $css_rules = $dfd_gallery_hover_style_class = $thumb_data_attr = $js_scripts = $extra_class_name = '';
			$sort_panel = false;
			...
			if(!empty($module_animation)) {
				...
				$data_atts .= ' data-animate-item=".dfd-gallery-single-item" data-animate-type="'.esc_attr($module_animation).'" ';
			}
